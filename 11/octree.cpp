...
// const char* Octree::OCTNode<T>::id_not_found::what() const noexcept
// {
...
template<typename T>
Octree::OCTNode<T>::OCTNode(u_int id, ONodePtr<T> parent, T value): ID(id), Value(value), parentPtr(parent) {}
template<typename T>
...
template<typename T>
Octree::ONodePtr<T> Octree::OCTNode<T>::addNode(u_int id, T value)
{
	if(hasChildren<8)
	{
...
template<typename T>
Octree::OCTREE<T>::OCTREE(u_int nodes, T value): Root(new OCTNode<T>(ID_Counter++,nullptr))
{
	while(ID_Counter<=nodes)
	{
...
template<typename T>
Octree::OCTREE<T>::OCTREE(Octree::UIntInitList nodes, T value): Root(new OCTNode<T>(ID_Counter++,nullptr))
{
	for(auto it: nodes)
	{
		addNode(it,value);
...
template<typename T>
Octree::ONodePtr<T> Octree::OCTREE<T>::addNode(u_int id, T value)
{
...
template<typename T>
void Octree::OCTREE<T>::addNode(Octree::UIntInitList IDs, T value)
{
	for(auto it: IDs)
	{
		addNode(it,value);
...
template<typename T>
T Octree::OCTREE<T>::getVal(u_int id) const
{
	ONodePtr<T> tmp=this->findID(id);
	return tmp ? tmp->ID : 0;
}
template<typename T>
T Octree::OCTREE<T>::setVal(u_int id, T value)
{
	ONodePtr<T> tmp=this->findID(id);
	return tmp ? (tmp->ID=value) : 0;
}