...
std::ostream& operator<< (std::ostream& stream, const class POSITION& P)
{
	...
	{
		stream << std::fixed << std::setprecision(1) << P.Szerokosc << " N ";
	}
	else if (P.Szerokosc < 0 && P.Szerokosc >= -90)
	{
		stream << std::setprecision(1) << P.Szerokosc << " S ";
	}
	else if( P.Szerokosc == 0)
	{
		stream << std::setprecision(1) << P.Szerokosc << ' ';
	}
	else stream<< "BŁĄD  ";
	if( P.Dlugosc <= 180 && P.Dlugosc >= 0)
	{
		stream << std::setprecision(1) << P.Dlugosc << " E";
	}
	else if (P.Dlugosc < 0 && P.Dlugosc >= -180)
	{
		stream << std::setprecision(1) << P.Dlugosc << " W";
	}
	else stream<< "BŁĄD";
	return stream;
}
...
double WORLD::distance(const PREDATOR &P, const VICTIM &V) throw(std::string)
{
	double d=P.getPosition().distance(V.getPosition());
	if(d==0)
	{
