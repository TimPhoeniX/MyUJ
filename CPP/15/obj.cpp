...
WORLD::WORLD(const unsigned int predators, const unsigned int victims)
{
	...
	std::uniform_real_distribution<double> gx(-20,20);
	std::uniform_real_distribution<double> gy(-10,10);
...
void WORLD::operator() (const double time, const int iterations, std::ostream& stream)
{
	this->simulate(time,iterations,stream);
}
