...
namespace CTL
{
	...
	public:
		...
		Matrix( const unsigned int r, const unsigned int c, T*const data)
		: Row(r), Col(c), Total(Row*Col), Val(data)
		{
		}
		...
		{
			Matrix<T> pMatrix(this->Row,this->Row);
		...
		{
			Matrix<T> pMatrix(this->Col,this->Col);
			for(unsigned int i = 0; i < this->Row; ++i)
			{
				pMatrix[permutation[i]][i]=1;
			}
		...
		Matrix<T> BackSubstitution(const Matrix<T>& vec)
		{
			if( vec.Col!=1 || this->Row != vec.Row ) throw std::out_of_range("Illegal vector type"); 
			Matrix<T> solution(this->Row,1);
			T var;
			for(int r = this->Row-1; r>-1; --r)
			{
				var = vec.Val[r];
				for(unsigned int c = r+1; c < this->Col; ++c)
				{
					var-=this->operator[](r)[c]*solution.Val[c];
				}
				solution.Val[r]=var/this->operator[](r)[r];
			}
			return solution;
		}
		
		Matrix<T> SolveWithLU(const Matrix<T>& vec)
		{
			if( vec.Col!=1 || this->Row != vec.Row ) throw std::out_of_range("Illegal vector type"); 
			Matrix<T> solution(this->Row,1);
			T var;
			...
			return this->BackSubstitution(solution);
		}
