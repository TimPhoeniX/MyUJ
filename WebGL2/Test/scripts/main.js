...
function init()
{
    ...
    let test = getObjectsFromFrame("#test");
    let mats = getMaterialsFromFrame('#testmtl');
    ...
    let attrs = makeAttrArray([
        new Attribute(gpu_positions_attrib_location,3,gl.FLOAT,false),
        new Attribute(gpu_normals_attrib_location,3,gl.FLOAT,false),
        new Attribute(gpu_tex_coord_attrib_location,2,gl.FLOAT,false)
    ]);

    for (let i = 0; i < test.length; i++) {
        const model = test[i];
        models.push(new Model(model.data,model.indices,attrs));
    }

    // pozycja kamery
    let cam_pos = new Float32Array([-1., 2., 5.]);
    ...
    mat4.lookAt(view_matrix, cam_pos, new Float32Array([0., 1., 0.]), new Float32Array([0., 1., 0.]));
    //mat4.lookAt(view_matrix, new Float32Array([0., -2., 2.]), new Float32Array([0., 0., 0.]), new Float32Array([0., 0., 1.]));
    var projection_matrix = mat4.create();
    mat4.perspective(projection_matrix, Math.PI/4., gl.drawingBufferWidth/gl.drawingBufferHeight, 0.01, 100);
    ...
    let point_light_data = new Float32Array([0., 2, 3., 512., 1., 1., 1.]);
    ...
    gl.bindTexture(gl.TEXTURE_2D, texture);
    gl.bindVertexArray(models[0].vao);
...
function draw()
{
    ...
    //mat4.rotateY(model_matrix, model_matrix, counter);
    mat4.scale(model_matrix,model_matrix, vec3.fromValues(0.2,0.2,0.2));
    ...
    //gl.drawElements(gl.TRIANGLES, model_v, gl.UNSIGNED_SHORT, 0);
...
var vs_source = `#version 300 es
    ...
    void main()
    {
        ...
        normal_ws = (transpose(inverse(matrices.model_matrix))*vec4(vertex_normal, 0.0)).xyz;
